...
class MatchPlayerResourceIT {
    ...
    private static final Integer DEFAULT_IS_WARNED = 1;
    private static final Integer UPDATED_IS_WARNED = 2;
    ...
    public static MatchPlayer createEntity(EntityManager em) {
        MatchPlayer matchPlayer = new MatchPlayer()
            ...
            .isWarned(DEFAULT_IS_WARNED)
            .miscData(DEFAULT_MISC_DATA);
        // Add required entity
        TeamPlayer teamPlayer;
        if (TestUtil.findAll(em, TeamPlayer.class).isEmpty()) {
            teamPlayer = TeamPlayerResourceIT.createEntity(em);
            em.persist(teamPlayer);
            em.flush();
        } else {
            teamPlayer = TestUtil.findAll(em, TeamPlayer.class).get(0);
        }
        matchPlayer.setTeamPlayer(teamPlayer);
        // Add required entity
        Match match;
    ...
    public static MatchPlayer createUpdatedEntity(EntityManager em) {
        MatchPlayer matchPlayer = new MatchPlayer()
            ...
            .isWarned(UPDATED_IS_WARNED)
            .miscData(UPDATED_MISC_DATA);
        // Add required entity
        TeamPlayer teamPlayer;
        if (TestUtil.findAll(em, TeamPlayer.class).isEmpty()) {
            teamPlayer = TeamPlayerResourceIT.createUpdatedEntity(em);
            em.persist(teamPlayer);
            em.flush();
        } else {
            teamPlayer = TestUtil.findAll(em, TeamPlayer.class).get(0);
    ...
    void createMatchPlayer() throws Exception {
        ...
        assertThat(testMatchPlayer.getIsYouth()).isEqualTo(DEFAULT_IS_YOUTH);
        assertThat(testMatchPlayer.getIsWarned()).isEqualTo(DEFAULT_IS_WARNED);
        assertThat(testMatchPlayer.getMiscData()).isEqualTo(DEFAULT_MISC_DATA);
    }
    ...
    void putNewMatchPlayer() throws Exception {
        ...
        assertThat(testMatchPlayer.getIsYouth()).isEqualTo(UPDATED_IS_YOUTH);
        assertThat(testMatchPlayer.getIsWarned()).isEqualTo(UPDATED_IS_WARNED);
        assertThat(testMatchPlayer.getMiscData()).isEqualTo(UPDATED_MISC_DATA);
    }
    ...
    void partialUpdateMatchPlayerWithPatch() throws Exception {
        ...
        partialUpdatedMatchPlayer.isWarned(UPDATED_IS_WARNED).miscData(UPDATED_MISC_DATA);
