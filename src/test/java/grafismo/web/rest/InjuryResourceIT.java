...
class InjuryResourceIT {
    ...
    private static final String DEFAULT_EST_HEALING_TIME = "AAAAAAAAAA";
    private static final String UPDATED_EST_HEALING_TIME = "BBBBBBBBBB";

    private static final Instant DEFAULT_EST_COMEBACK_DATE = Instant.ofEpochMilli(0L);
    private static final Instant UPDATED_EST_COMEBACK_DATE = Instant.now().truncatedTo(ChronoUnit.MILLIS);
    ...
    @Mock
    private InjuryRepository injuryRepositoryMock;

    @Autowired
    private InjuryMapper injuryMapper;

    @Mock
    private InjuryService injuryServiceMock;
    ...
    public static Injury createEntity(EntityManager em) {
        Injury injury = new Injury()
            ...
            .estHealingTime(DEFAULT_EST_HEALING_TIME)
            .estComebackDate(DEFAULT_EST_COMEBACK_DATE)
    ...
    public static Injury createUpdatedEntity(EntityManager em) {
        Injury injury = new Injury()
            ...
            .estHealingTime(UPDATED_EST_HEALING_TIME)
            .estComebackDate(UPDATED_EST_COMEBACK_DATE)
    ...
    void createInjury() throws Exception {
        ...
        assertThat(testInjury.getEstHealingTime()).isEqualTo(DEFAULT_EST_HEALING_TIME);
        assertThat(testInjury.getEstComebackDate()).isEqualTo(DEFAULT_EST_COMEBACK_DATE);
    ...
    void getAllInjuries() throws Exception {
        ...
        restInjuryMockMvc
            ...
            .andExpect(jsonPath("$.[*].estHealingTime").value(hasItem(DEFAULT_EST_HEALING_TIME)))
            .andExpect(jsonPath("$.[*].estComebackDate").value(hasItem(DEFAULT_EST_COMEBACK_DATE.toString())))
            .andExpect(jsonPath("$.[*].reason").value(hasItem(DEFAULT_REASON)));
    }

    @SuppressWarnings({ "unchecked" })
    void getAllInjuriesWithEagerRelationshipsIsEnabled() throws Exception {
        when(injuryServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restInjuryMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(injuryServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllInjuriesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(injuryServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void getInjury() throws Exception {
        ...
        restInjuryMockMvc
            ...
            .andExpect(jsonPath("$.estHealingTime").value(DEFAULT_EST_HEALING_TIME))
            .andExpect(jsonPath("$.estComebackDate").value(DEFAULT_EST_COMEBACK_DATE.toString()))
    ...
    void putNewInjury() throws Exception {
        ...
        assertThat(testInjury.getEstHealingTime()).isEqualTo(UPDATED_EST_HEALING_TIME);
        assertThat(testInjury.getEstComebackDate()).isEqualTo(UPDATED_EST_COMEBACK_DATE);
    ...
    void partialUpdateInjuryWithPatch() throws Exception {
        ...
        partialUpdatedInjury.estHealingTime(UPDATED_EST_HEALING_TIME).estComebackDate(UPDATED_EST_COMEBACK_DATE).reason(UPDATED_REASON);
