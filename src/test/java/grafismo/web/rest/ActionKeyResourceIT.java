...
class ActionKeyResourceIT {
    ...
    @Mock
    private ActionKeyRepository actionKeyRepositoryMock;

    @Autowired
    private ActionKeyMapper actionKeyMapper;

    @Mock
    private ActionKeyService actionKeyServiceMock;
    ...
    public static ActionKey createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, GraphicElement.class).isEmpty()) {
            graphicElement = GraphicElementResourceIT.createEntity(em);
            em.persist(graphicElement);
            em.flush();
        } else {
            graphicElement = TestUtil.findAll(em, GraphicElement.class).get(0);
        }
        actionKey.setGraphicElement(graphicElement);
        return actionKey;
    }
    ...
    public static ActionKey createUpdatedEntity(EntityManager em) {
        ...
        GraphicElement graphicElement;
        if (TestUtil.findAll(em, GraphicElement.class).isEmpty()) {
            graphicElement = GraphicElementResourceIT.createUpdatedEntity(em);
            em.persist(graphicElement);
            em.flush();
        } else {
            graphicElement = TestUtil.findAll(em, GraphicElement.class).get(0);
    ...
    void checkActionIsRequired() throws Exception {
        int databaseSizeBeforeTest = actionKeyRepository.findAll().size();
        // set the field null
        actionKey.setAction(null);

        // Create the ActionKey, which fails.
        ActionKeyDTO actionKeyDTO = actionKeyMapper.toDto(actionKey);
        ...
        List<ActionKey> actionKeyList = actionKeyRepository.findAll();
        assertThat(actionKeyList).hasSize(databaseSizeBeforeTest);
    }

    @Test
    @Transactional
    void checkKeysIsRequired() throws Exception {
        int databaseSizeBeforeTest = actionKeyRepository.findAll().size();
        // set the field null
        actionKey.setKeys(null);
    ...
    void getAllActionKeysWithEagerRelationshipsIsEnabled() throws Exception {
        when(actionKeyServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restActionKeyMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(actionKeyServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllActionKeysWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(actionKeyServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
