...
public class TeamRepositoryWithBagRelationshipsImpl implements TeamRepositoryWithBagRelationships {
    ...
    public Optional<Team> fetchBagRelationships(Optional<Team> team) {
        return team.map(this::fetchVenues);
    }
    ...
    public List<Team> fetchBagRelationships(List<Team> teams) {
        return Optional.of(teams).map(this::fetchVenues).orElse(Collections.emptyList());
    }

    Team fetchVenues(Team result) {
        return entityManager
            .createQuery("select team from Team team left join fetch team.venues where team is :team", Team.class)
            .setParameter("team", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Team> fetchVenues(List<Team> teams) {
        return entityManager
            .createQuery("select distinct team from Team team left join fetch team.venues where team in :teams", Team.class)
