...
public interface MatchRepository extends MatchRepositoryWithBagRelationships, JpaRepository<Match, Long> {
    ...
    @Query(
        value = "select distinct jhiMatch from Match jhiMatch left join fetch jhiMatch.homeTeam left join fetch jhiMatch.awayTeam left join fetch jhiMatch.venue left join fetch jhiMatch.matchday",
        countQuery = "select count(distinct jhiMatch) from Match jhiMatch"
    )
    Page<Match> findAllWithToOneRelationships(Pageable pageable);

    @Query(
        "select distinct jhiMatch from Match jhiMatch left join fetch jhiMatch.homeTeam left join fetch jhiMatch.awayTeam left join fetch jhiMatch.venue left join fetch jhiMatch.matchday"
    )
    List<Match> findAllWithToOneRelationships();

    @Query(
        "select jhiMatch from Match jhiMatch left join fetch jhiMatch.homeTeam left join fetch jhiMatch.awayTeam left join fetch jhiMatch.venue left join fetch jhiMatch.matchday where jhiMatch.id =:id"
    )
