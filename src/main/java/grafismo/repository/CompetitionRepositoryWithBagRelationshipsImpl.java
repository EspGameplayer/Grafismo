...
public class CompetitionRepositoryWithBagRelationshipsImpl implements CompetitionRepositoryWithBagRelationships {
    ...
    public Optional<Competition> fetchBagRelationships(Optional<Competition> competition) {
        return competition.map(this::fetchTeams).map(this::fetchReferees);
    }
    ...
    public List<Competition> fetchBagRelationships(List<Competition> competitions) {
        return Optional.of(competitions).map(this::fetchTeams).map(this::fetchReferees).orElse(Collections.emptyList());
    }
    ...
    Competition fetchReferees(Competition result) {
        return entityManager
            .createQuery(
                "select competition from Competition competition left join fetch competition.referees where competition is :competition",
                Competition.class
            )
            .setParameter("competition", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
    ...
    List<Competition> fetchReferees(List<Competition> competitions) {
        return entityManager
            .createQuery(
                "select distinct competition from Competition competition left join fetch competition.referees where competition in :competitions",
                Competition.class
            )
            .setParameter("competitions", competitions)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
