...
public interface SystemConfigurationRepository extends JpaRepository<SystemConfiguration, Long> {
    ...
    @Query(
        value = "select distinct systemConfiguration from SystemConfiguration systemConfiguration left join fetch systemConfiguration.currentSeason",
    ...
    @Query(
        "select distinct systemConfiguration from SystemConfiguration systemConfiguration left join fetch systemConfiguration.currentSeason"
    )
    List<SystemConfiguration> findAllWithToOneRelationships();

    @Query(
        "select systemConfiguration from SystemConfiguration systemConfiguration left join fetch systemConfiguration.currentSeason where systemConfiguration.id =:id"
    )
