...
public interface TeamStaffMemberRepository extends JpaRepository<TeamStaffMember, Long> {
    ...
    @Query(
        value = "select distinct teamStaffMember from TeamStaffMember teamStaffMember left join fetch teamStaffMember.team left join fetch teamStaffMember.staffMember",
    ...
    @Query(
        "select distinct teamStaffMember from TeamStaffMember teamStaffMember left join fetch teamStaffMember.team left join fetch teamStaffMember.staffMember"
    )
    List<TeamStaffMember> findAllWithToOneRelationships();

    @Query(
        "select teamStaffMember from TeamStaffMember teamStaffMember left join fetch teamStaffMember.team left join fetch teamStaffMember.staffMember where teamStaffMember.id =:id"
    )
