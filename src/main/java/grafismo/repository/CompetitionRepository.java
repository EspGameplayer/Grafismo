...
public interface CompetitionRepository extends CompetitionRepositoryWithBagRelationships, JpaRepository<Competition, Long> {
    ...
    @Query(
        value = "select distinct competition from Competition competition left join fetch competition.parent left join fetch competition.country",
    ...
    @Query(
        "select distinct competition from Competition competition left join fetch competition.parent left join fetch competition.country"
    )
    List<Competition> findAllWithToOneRelationships();

    @Query(
        "select competition from Competition competition left join fetch competition.parent left join fetch competition.country where competition.id =:id"
    )
