...
public class LineupResource {
    ...
    public ResponseEntity<List<LineupDTO>> getAllLineups(
        ...
        if ("homematch-is-null".equals(filter)) {
            log.debug("REST request to get all Lineups where homeMatch is null");
            return new ResponseEntity<>(lineupService.findAllWhereHomeMatchIsNull(), HttpStatus.OK);
        }

        if ("awaymatch-is-null".equals(filter)) {
            log.debug("REST request to get all Lineups where awayMatch is null");
            return new ResponseEntity<>(lineupService.findAllWhereAwayMatchIsNull(), HttpStatus.OK);
        }
