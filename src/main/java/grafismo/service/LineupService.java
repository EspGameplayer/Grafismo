...
public class LineupService {
    ...
    /**
     *  Get all the lineups where HomeMatch is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<LineupDTO> findAllWhereHomeMatchIsNull() {
        log.debug("Request to get all lineups where HomeMatch is null");
        return StreamSupport
            .stream(lineupRepository.findAll().spliterator(), false)
            .filter(lineup -> lineup.getHomeMatch() == null)
            .map(lineupMapper::toDto)
            .collect(Collectors.toCollection(LinkedList::new));
    }

    /**
     *  Get all the lineups where AwayMatch is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<LineupDTO> findAllWhereAwayMatchIsNull() {
        log.debug("Request to get all lineups where AwayMatch is null");
        return StreamSupport
            .stream(lineupRepository.findAll().spliterator(), false)
            .filter(lineup -> lineup.getAwayMatch() == null)
            .map(lineupMapper::toDto)
            .collect(Collectors.toCollection(LinkedList::new));
    }
