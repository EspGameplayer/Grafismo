...
public class ActionKeyService {
    ...
    /**
     * Get all the actionKeys with eager load of many-to-many relationships.
     *
     * @return the list of entities.
     */
    public Page<ActionKeyDTO> findAllWithEagerRelationships(Pageable pageable) {
        return actionKeyRepository.findAllWithEagerRelationships(pageable).map(actionKeyMapper::toDto);
    }
    ...
    public Optional<ActionKeyDTO> findOne(Long id) {
        ...
        return actionKeyRepository.findOneWithEagerRelationships(id).map(actionKeyMapper::toDto);
    }
