...
public interface TeamStaffMemberMapper extends EntityMapper<TeamStaffMemberDTO, TeamStaffMember> {
    ...
    @Mapping(target = "staffMember", source = "staffMember", qualifiedByName = "staffMemberGraphicsName")
    ...
    @Named("staffMemberGraphicsName")
    @BeanMapping(ignoreByDefault = true)
    @Mapping(target = "id", source = "id")
    @Mapping(target = "graphicsName", source = "graphicsName")
    StaffMemberDTO toDtoStaffMemberGraphicsName(StaffMember staffMember);
}
